...
var Updater = {
  ...
  download: function(callback) {
    ...
    var url = this.update.source,
      update_sha1 = this.update.sha1;

    this.log("Downloading " + url);

    this.setup.downloadRequest = request(
      {
        uri: url,
        encoding: null
      },
    ...
    progress(this.setup.downloadRequest, {
        ...
        if (process.platform === "win32") {
          ...
          try {
            fs.writeFileSync(
              WindowsUpdater,
              fs.readFileSync(
                `${AppPathFolder}app.asar/node_modules/${
          ...
          winArgs = `${JSON.stringify(WindowsUpdater)} ${JSON.stringify(
            updateAsar
          )} ${JSON.stringify(appAsar)} ${JSON.stringify(executable)}`;
          Updater.log(winArgs);
          ...
          spawn(
            ...
            [
              "-c",
              [
                "cd " + JSON.stringify(AppPathFolder),
                `mv -f ${UPDATE_FILE} app.asar`,
                executable
              ].join(" && ")
            ],
            { detached: true }
  ...
  cancel() {
    this.log("cancel: 取消更新");
    this.setup.downloadRequest.abort();
  }
};
